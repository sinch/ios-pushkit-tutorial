...
The biggest challenge when writing communications apps is providing a quick experience that won’t drain your users’ battery. Today, we are going to investigate the new PushKit framework and make it run with our [Voice API](http://www.sinch.com/products/calling/voice-api/). It will require some beta installs and server-side code, so expect to spend around 45 to 60 minutes here. This tutorial assumes that you are familiar with the old push system and how to create certificates.
